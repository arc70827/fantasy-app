...
if (!file.exists(cache_path)) {
  stop(
    paste0(
      "Missing prepared app cache: ",
      cache_path,
      ". Run scripts/build_app_cache.R before starting the app."
    ),
    call. = FALSE
  )
}

app_cache <- readRDS(cache_path)
...
if (length(missing_cache_objects) > 0) {
  stop(
    "The prepared app cache is missing required objects: ",
    paste(missing_cache_objects, collapse = ", "),
    ". Rebuild it before starting the app.",
    call. = FALSE
  )
}

matchups <- app_cache$matchups
players <- app_cache$players
team_names <- app_cache$team_names
regular_season_matchups <- app_cache$regular_season_matchups
regular_season_players <- app_cache$regular_season_players
years <- app_cache$years
managers <- app_cache$managers
all_teams <- app_cache$all_teams
all_positions <- app_cache$all_positions
all_slots <- app_cache$all_slots
latest_year <- app_cache$latest_year
latest_week <- app_cache$latest_week
pair_games <- app_cache$pair_games
manager_finishes <- app_cache$manager_finishes
playoff_years <- app_cache$playoff_years
latest_playoff_year <- app_cache$latest_playoff_year
weekly_expected_wins <- app_cache$weekly_expected_wins
weekly_lineup_analytics <- app_cache$weekly_lineup_analytics
...
manager_range_contains <- function(
  ...
  ends_after_or_on <-
    ...
    (
      end_year_value == year_value &
      end_week_value >= week_value
    )

  starts_before_or_on & ends_after_or_on
}

resolve_team_name_one <- function(
    manager_value,
    year_value,
...
manager_season_finish_info <- function(
    manager_value,
    year_value
) {
  ...
  team_name <- resolve_team_name_one(
    manager_value,
    year_value
  )
  ...
  finish_label <- case_when(
    ...
    nrow(season_games) > 0 &&
      !made_postseason &&
      season_in_progress ~ "In Progress",
    nrow(season_games) > 0 &&
      !made_postseason ~ "Missed Playoffs",
    TRUE ~ "Unavailable"
  )
...
h2h_summary <- function(
  ...
  games <- matchups |>
    filter(
      (
        manager == manager_one &
        opposing_manager == manager_two
      ) |
      (
        manager == manager_two &
        opposing_manager == manager_one
      )
    ) |>
    filter(
      year < through_year |
  ...
  tibble(
    manager_one_wins = sum(
      games$manager == manager_one &
      games$win == 1,
      na.rm = TRUE
    ),
    manager_two_wins = sum(
      games$manager == manager_two &
      games$win == 1,
      na.rm = TRUE
    ),
    games = nrow(games) / 2,
    manager_one_avg = mean(
      games$points_for[
        games$manager == manager_one
      ],
      na.rm = TRUE
    ),
    manager_two_avg = mean(
      games$points_for[
        games$manager == manager_two
      ],
      na.rm = TRUE
